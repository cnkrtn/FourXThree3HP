...
namespace AudioStream
{
    ...
    public static partial class FMODSystemsManager
    {
        ...
        )
        {
            ...
            {
                FMODSystemsManager.system_NoSound_NRT =
                    new FMODSystem(withSpeakerMode, withNumOfRawSpeakers, true, false);
                AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                    "Created FMOD system for non realtime decoding {0}",
                    FMODSystemsManager.system_NoSound_NRT.System.handle);
            }
        ...
        )
        {
            ...
            if (--FMODSystemsManager.system_NoSound_NRT_refCount < 1)
            {
                ...
                if (fmodsystem != null)
                {
                    fmodsystem.Release();
                    AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                        "Released system for non realtime decoding {0}", fmodsystem.System.handle);
                }
        ...
        )
        {
            ...
            {
                FMODSystemsManager.system_NoSound_RT =
                    new FMODSystem(withSpeakerMode, withNumOfRawSpeakers, true, true);
                AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                    "Created FMOD system for Unity AudioSources {0}",
                    FMODSystemsManager.system_NoSound_RT.System.handle);
            }
        ...
        )
        {
            ...
            if (--FMODSystemsManager.system_NoSound_RT_refCount < 1)
            {
                ...
                if (fmodsystem != null)
                {
                    fmodsystem.Release();
                    AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                        "Released system for Unity AudioSources {0}", fmodsystem.System.handle);
                }
        ...
        )
        {
            ...
            if (!FMODSystemsManager.systems_direct_sound.Contains(currentSystem))
            {
                ...
                AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                    "Created FMOD system for direct/minimal FMOD playback {0}", result.System.handle);
            }
        ...
        )
        {
            ...
            AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError,
                "Released system for direct/minimal FMOD playback {0}", fmodsystem.System.handle);
        ...
        static FMOD.RESULT DEBUG_CALLBACK(FMOD.DEBUG_FLAGS flags, FMOD.StringWrapper file, int line,
            FMOD.StringWrapper func, FMOD.StringWrapper message)
        {
            var msg = (string)message;
            if (!msg.Contains("FMOD_RESULT = 63")) // missing tags in stream is reported for every frame
                Debug.LogFormat("{0} {1}:{2} {3} {4}", flags, System.IO.Path.GetFileName((string)file), line,
        ...
        public static void InitializeFMODDiagnostics(FMOD.DEBUG_FLAGS flags)
        {
            if (FMODSystemsManager.DEBUG_CALLBACK_DELEGATE == null)
            {
                ...
                Debug.LogFormat("new FMODSystemsManager.DEBUG_CALLBACK_DELEGATE {0}",
